...
HitRecord BoundingVolumeHierarchy::GetIntersection(const Ray& ray, const Object* hit_obj) const {
  HitRecord hit_record;
  hit_record.t = kInf;
...
bool BoundingVolumeHierarchy::GetIntersection(const Ray& ray, float tmax,const Object* hit_obj) const {
  float tmin = kInf;
  const BoundingBox bounding_box = tree_->bounding_box;
  const float t = bounding_box.DoesIntersect(ray);
