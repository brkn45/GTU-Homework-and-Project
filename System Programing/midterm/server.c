        ...
        while(1){
            ...
            if(tryConnect == 1){
                ...
                if(fork()==0){
                    ...
                    write(logfd,"\nChild Process Created.\nChild Process Pid: ",strlen("Child Process Created.\nChild Process Pid: "));
                    write(logfd,pidSrt,strlen(pidSrt));
                    while (1) {
                        //printf("file open during  arr:%s\n",arr);
                        childFd = open(arr, O_RDONLY);
                        if(childFd < 0){
                            printf("dosya arr:%s\n",arr);
                            printf("file open error:%d\n",childFd);
                            write(logfd,"Fifo file Cant Open\n",strlen("Fifo file Cant Open\n"));
                            sem_post(semaphore);
                            exit(1);
                        }
                        int serverByte =0;
                    
                        serverByte = read(childFd, message, sizeof(message));
                        printf("Client Comment>>%s", message);
                        write(logfd,"\n>>Client Comment: ",strlen("\n>>Client Comment: "));
                        write(logfd,message,strlen(message));
                        if(serverByte == -1){
                            printf("Serverr Error\n");
                            write(logfd,"Server Error.\n",14);
                            sem_post(semaphore);
                            exit(1);
                        }
                        ...
                        if(strncmp(message,"list",4) == 0){
                            ...
                            write(logfd,"Child and Prarent Process Killed\n",strlen("Child and Prarent Process Killed\n")+1);
                            write(logfd,"kill signal from client terminating...\n",strlen("kill signal from client terminating...\n"));
                            printf("kill signal from client0%d.. terminating...\n",childNum);
                            printf(">>Child and Prarent Process Killed\n");
                            sem_post(semaphore);
                            close(logfd);
                    ...
                    sem_post(semaphore);
                    exit(EXIT_SUCCESS);  // Important: child should exit here!
