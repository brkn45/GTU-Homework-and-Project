...
namespace RayTracer
{
	...
	public class Vertex
	{
		...
		//Vertex operator * (float f)					// R = Ro + Rd * t;
		//{
		//	return Vertex(f*X, f*Y, f*Z);
		//}

		Vertex operator / (float f)						// (p - Center) / Radius
		{
			return Vertex(X / f, Y / f, Z / f);
		}
	};

	Vertex operator * (float f, Vertex p)				// R = Ro + t * Rd;
	{
		return Vertex(f * p.X, f * p.Y, f * p.Z);
	}

	Vertex operator * (Vertex p, float f)				// R = Ro + Rd * t;
	{
		return Vertex(f * p.X, f * p.Y, f * p.Z);
	}
}
