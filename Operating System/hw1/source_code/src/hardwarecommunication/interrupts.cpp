...
{
    //esp = (uint32_t)interruptManager->taskManager->Schedule((CPUState*)esp);
            //interruptManager->taskManager->printProcessTable();
    return esp;
}
      ...
      programmableInterruptControllerSlaveDataPort(0xA1)
{
    ...
    SetInterruptDescriptorTableEntry(                          0x81, CodeSegment, &HandleInterruptRequest0x80, 0, IDT_INTERRUPT_GATE);
...
bool InterruptManager::setWaitProcessForAllChild(CPUState* cpustate){
    return taskManager->parentProcessWaitAllChild(cpustate);
}
...
int InterruptManager::setPriortyIM(CPUState* cpustate){
    
    return taskManager->setPriorityTaskManager(cpustate);
}

uint32_t InterruptManager::scheduleTask(CPUState* cpustate){
    uint32_t esp = (uint32_t)taskManager->Schedule(cpustate); // Burayı düzeltmek gerekiyor normal scheduling için
    //uint32_t esp = (uint32_t)taskManager->prioritySchedule(cpustate);
    taskManager->printProcessTable();
    return esp;
    
}

myos::common::uint32_t InterruptManager::lowerPriortytySchedule(myos::common::uint32_t esp){
    uint32_t esp_main = taskManager->lowerLevelSchedule(esp);
    taskManager->printProcessTable();
    return esp_main;
}